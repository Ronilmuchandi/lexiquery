...
#
# THE RAG FLOW:
# User Question → Hybrid Retrieve Clauses → Send to Groq → Get Answer
#
# SIMPLE ANALOGY:
# Imagine you have a lawyer (Groq) and a smart filing cabinet
# (ChromaDB). When you ask a question:
# 1. Assistant finds relevant files (hybrid retrieval)
...
# Our hybrid retrieval function (BM25 + Vector search)
from src.rag.hybrid_retriever import hybrid_retrieve as retrieve_relevant_clauses

# Centralized error handlers
from src.handlers.error_handler import handle_llm_error, handle_retrieval_error
...
def answer_legal_question(
    ...
    # -------------------------------------------------------
    # STEP 1: RETRIEVE
    # Find the most relevant clauses using hybrid search
    # (BM25 keyword search + semantic vector search combined)
    # -------------------------------------------------------
    ...
    try:
        ...
        error = handle_retrieval_error(e)
        return {
            "answer": f"❌ {error['message']}. {error['suggestion']}",
            "clauses_used": [],
            "relevant_clauses": []
        }
    ...
    if not relevant_clauses:
        error = handle_retrieval_error(
            Exception("No relevant clauses found")
        )
        return {
            "answer": f"❌ {error['message']}. {error['suggestion']}",
            "clauses_used": [],
